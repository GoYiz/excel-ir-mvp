...
def run_bench(max_total_seconds: float = DEFAULT_MAX_SECONDS):
    results = []
    results.append(run(['excel-ir', 'parse', 'tests/fixtures/complex_report.xlsx', 'bench_ir.json']))
    results.append(run(['excel-ir', 'rebuild', 'bench_ir.json', 'bench_rebuilt.xlsx']))
    results.append(run(['excel-ir', 'diff', 'tests/fixtures/complex_report.xlsx', 'bench_rebuilt.xlsx', 'bench_diff.json']))
    results.append(run(['excel-ir', 'patch', 'bench_ir.json', 'tests/fixtures/v08_patch.json', 'bench_patched_ir.json', '--log', 'bench_tx.json']))
