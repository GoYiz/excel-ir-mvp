...
def list_samples(config: dict):
    ...
    for sample in config.get('samples', []):
        samples.append({'name': sample.get('name'), 'category': sample.get('category', 'uncategorized'), 'xlsx': sample.get('xlsx'), 'patch': sample.get('patch')})
    cats = {}
    for s in samples:
...
def run_corpus(config: dict):
    ...
    for sample in config.get('samples', []):
        ...
        item = {'name': name, 'category': category, 'parse_ok': p1.returncode == 0, 'metadata_export_ok': bool(pmeta and pmeta.returncode == 0), 'metadata_verify_xlsx_ok': bool(pverify and pverify.returncode == 0), 'rebuild_ok': bool(p2 and p2.returncode == 0), 'diff_ok': bool(p3 and p3.returncode == 0), 'diff_count': diff_count}
        if metadata.exists():
            try:
...
def render_summary_html(summary: dict, title: str = 'Excel IR Corpus Report') -> str:
    ...
    for r in summary.get('results', []):
        ok = bool(r.get('parse_ok') and r.get('rebuild_ok') and r.get('diff_count') == 0)
        vals = [r.get('name'), r.get('category'), ok, r.get('diff_count'), r.get('metadata_tables'), r.get('metadata_verify_xlsx_ok')]
        rows.append('<tr>' + ''.join(f'<td>{html.escape(str(v))}</td>' for v in vals) + '</tr>')
    cats = [f"<li><b>{html.escape(name)}</b>: {c.get('ok')}/{c.get('count')} ok, diff_total={c.get('diff_count_total')}</li>" for name, c in sorted((summary.get('categories') or {}).items())]
    return f"""<!doctype html><meta charset='utf-8'><title>{html.escape(title)}</title><style>body{{font-family:-apple-system,BlinkMacSystemFont,sans-serif;margin:2rem}}table{{border-collapse:collapse}}td,th{{border:1px solid #ddd;padding:.35rem .5rem}}th{{background:#f5f5f5}}</style><h1>{html.escape(title)}</h1><p>ok: <b>{summary.get('ok')}</b></p><h2>Categories</h2><ul>{''.join(cats)}</ul><h2>Samples</h2><table><thead><tr><th>Name</th><th>Category</th><th>OK</th><th>Diffs</th><th>Metadata tables</th><th>XLSX metadata verify</th></tr></thead><tbody>{''.join(rows)}</tbody></table>"""
...
def main():
    ...
    p = sub.add_parser('list'); p.add_argument('--config')
    p = sub.add_parser('report'); p.add_argument('summary_json'); p.add_argument('html'); p.add_argument('--title', default='Excel IR Corpus Report')
    ap.add_argument('--config')
    args = ap.parse_args()
    cmd = args.cmd or 'run'
    if cmd == 'list':
        print(json.dumps(list_samples(load_config(args.config)), ensure_ascii=False, indent=2)); return
    if cmd == 'report':
        summary = json.loads(Path(args.summary_json).read_text(encoding='utf-8'))
        write_report(summary, args.html, args.title)
        print(json.dumps({'ok': True, 'output': args.html}, ensure_ascii=False)); return
